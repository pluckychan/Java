...
//                googleDriveFileHolder.setSize(result.getFiles().get(0).getSize());
    ...
    public Task<Void> uploadFile2Folder(final java.io.File localFile, final String mimeType, final String vsTargetFolder, final Boolean vbRenameExistingFile, final String[] vaRenExistingFileName) {
        return Tasks.call(mExecutor, new Callable<Void>() {
            @Override
            public Void call() throws Exception {
                // Retrieve the metadata as a File object.

                vaRenExistingFileName[0] = "";      // renamed file name if backup file already exists
                String sTargetFolderId = "", sTargetFileId = "";
                String sDuplicateFileModifiedTime = "";
                FileList result = mDriveService.files().list()
                        .setQ("mimeType = '" + DriveFolder.MIME_TYPE + "'")
                        .setSpaces("drive")
                        .execute();

                for (int i = 0; i < result.getFiles().size(); i++) {                    // search Drive Folder
                    String sTmpName = result.getFiles().get(i).getName();
                    if (vsTargetFolder.equals(sTmpName)) {
                        /*GoogleDriveFileHolder googleDriveFileHolder = new GoogleDriveFileHolder();
                        googleDriveFileHolder.setId(result.getFiles().get(i).getId());
                        googleDriveFileHolder.setName(result.getFiles().get(i).getName());
                        googleDriveFileHolderList.add(googleDriveFileHolder);*/
                        sTargetFolderId = result.getFiles().get(i).getId();              // update Drive Id
                        break;
                    }
                }

                if (sTargetFolderId.equals("")) {
                    List<String> root;
                    root = Collections.singletonList("root");
                    File metadata = new File()
                            .setParents(root)
                            .setMimeType(DriveFolder.MIME_TYPE)
                            .setName(vsTargetFolder);
                    File googleFile = mDriveService.files().create(metadata).execute();
                    sTargetFolderId = googleFile.getId();
                    //throw new Exception(String.format("%s folder not found", vsTargetFolder));
                }

                if (!sTargetFolderId.equals("")){
                    String sSearchKey = String.format("%s = '%s' and '%s' in %s and trashed = false",
                            SearchableField.MIME_TYPE, mimeType, sTargetFolderId, SearchableField.PARENTS);
                    ...
                    for (int i = 0; i < result.getFiles().size(); i++) {
                        String sTmpName = result.getFiles().get(i).getName();
                        if (localFile.getName().equals((sTmpName))) {
                            sTargetFileId = result.getFiles().get(i).getId();
                            sDuplicateFileModifiedTime = result.getFiles().get(i).getModifiedTime().toString(); // memorize the modified time for duplicate file
                        }
                    }
                }

                // rename existing file if file already exist
                if (vbRenameExistingFile) {
                    if (!sTargetFileId.equals("")) {
                        File renFile = new File();
                        String sCurrTime = (sDuplicateFileModifiedTime.equals("")) ? DateFormat.format("yyyy-MM-dd.HH:mm:ss", Calendar.getInstance().getTime()).toString() :
                                sDuplicateFileModifiedTime;
                        renFile.setName(localFile.getName() + "." + sCurrTime);
                        vaRenExistingFileName[0] = localFile.getName() + "." + sCurrTime;   // if file already exits, rename the existing file and return the new file name
                        Files.Update updFile = mDriveService.files().update(sTargetFileId, renFile);
                        updFile.setFields("name");
                        updFile.execute();
                    }
                }
                ...
                File metadata = new File()
                        .setParents(Collections.singletonList(sTargetFolderId))
                        .setMimeType(mimeType)
                        .setName(localFile.getName());
    ...
    // function to list out similar files from File location
    public Task<Void> listFileFromPath(final java.io.File fileSaveLocation, final String vsFolderName, final String vsFileName, final String mimeType,
                                       final ArrayList<String> raOutFileList, final ArrayList<String> raOutFileIdList) throws IOException {
        return Tasks.call(mExecutor, new Callable<Void>() {
            @Override
            public Void call() throws Exception {

                String sTargetDriveId = "";
                String sTargetFileId = "";
                raOutFileList.clear();
                raOutFileIdList.clear();
                FileList result = mDriveService.files().list()
                        .setQ("mimeType = '" + DriveFolder.MIME_TYPE + "'")
                        .setSpaces("drive")
                        .execute();
                for (int i = 0; i < result.getFiles().size(); i++) {
                    String sTmpName = result.getFiles().get(i).getName();
                    if (vsFolderName.equals(sTmpName)) {
                        /*GoogleDriveFileHolder googleDriveFileHolder = new GoogleDriveFileHolder();
                        googleDriveFileHolder.setId(result.getFiles().get(i).getId());
                        googleDriveFileHolder.setName(result.getFiles().get(i).getName());
                        googleDriveFileHolderList.add(googleDriveFileHolder);*/
                        sTargetDriveId = result.getFiles().get(i).getId();              // update Drive Id
                        break;
                    }
                }
                if (!sTargetDriveId.equals("")) {
                    //String sSearchKey = String.format("%s = 'text/plain' and '%s' in %s and trashed = false",
                    String sSearchKey = String.format("%s = '%s' and '%s' in %s and trashed = false",
                            SearchableField.MIME_TYPE, mimeType, sTargetDriveId, SearchableField.PARENTS);
                    result = mDriveService.files().list()
                            .setQ(sSearchKey)
                            .setSpaces("drive")
                            .setFields("files(id, name,size,createdTime,modifiedTime,starred)")
                    ...
                    for (int i = 0; i < result.getFiles().size(); i++) {
                        String sTmpName = result.getFiles().get(i).getName();
                        if (sTmpName.indexOf(vsFileName) == 0) {    /* backup files */
                            raOutFileList.add(sTmpName);
                            sTargetFileId = result.getFiles().get(i).getId();
                            raOutFileIdList.add(sTargetFileId);
                        }
                    }
                }
                return null;
    ...
    public Task<Void> getFileById(final String vsFileId, final java.io.File fileSaveLocation) {
        return Tasks.call(mExecutor, new Callable<Void>() {
            @Override
            public Void call() throws Exception {
            OutputStream outputStream = new FileOutputStream(fileSaveLocation);
            mDriveService.files().get(vsFileId).executeMediaAndDownloadTo(outputStream);
                return null;
            }
        });
    }
    ...
    public Task<Void> downloadFileFromPath(final java.io.File fileSaveLocation, final String vsFolderName, final String vsFileName, final String mimeType) {
        return Tasks.call(mExecutor, new Callable<Void>() {
            @Override
            public Void call() throws Exception {
                //List<GoogleDriveFileHolder> googleDriveFileHolderList = new ArrayList<>();
                ...
                if (sTargetDriveId.equals("")) {
                    throw new Exception(String.format("%s folder not found", vsFolderName));
                } else {
                    //String sSearchKey = String.format("%s = 'text/plain' and '%s' in %s and trashed = false",
                    String sSearchKey = String.format("%s = '%s' and '%s' in %s and trashed = false",
                            SearchableField.MIME_TYPE, mimeType, sTargetDriveId, SearchableField.PARENTS);
                ...
                if (!sTargetDriveId.equals("")) {
                    if (!sTargetFileId.equals("")) {
                        OutputStream outputStream = new FileOutputStream(fileSaveLocation);
                        mDriveService.files().get(sTargetFileId).executeMediaAndDownloadTo(outputStream);
                    } else {
                        throw new Exception(String.format("%s - file not found in Google Drive", vsFileName));
                    }
                } else {
                    throw new Exception(String.format("%s - folder not found in Google Drive", vsFolderName));
                }
                return null;
            }
        });
